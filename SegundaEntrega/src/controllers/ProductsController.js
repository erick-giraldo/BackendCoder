  ...
  static async updateProduct(req, res) {
    try {
      ...
      if (isEmpty(productData))
        throw new Error(
          JSON.stringify({
            detail: "No se ha ingresado nungún elemento a actualizar",
          })
        );
      ...
      if (isEmpty(productById))
        return res
          .status(404)
          .json({ message: "El producto a editar no existe" });
      ...
      if (!isEmpty(productData.code)) {
        const productByCode = await ProductModel.findOne({
          code: productData.code,
        });
        if (!isEmpty(productByCode) && productByCode.id !== pid) {
          throw new Error(
            JSON.stringify({
              detail: `El código ${productData.code} ya se encuentra regitrado`,
            })
          );
        }
      }
